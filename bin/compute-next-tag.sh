...
#
# Tags look like `v<Forgejo version>-<release>`:
#
# - if defaults/main.yml points at a Forgejo version that has never been
#   released, the release counter restarts at 0 (`v16.0.4-0`)
# - otherwise the counter is incremented (`v16.0.4-1`), but only if something
...
version="$(sed -nE 's|^forgejo_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Forgejo version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `16.0.3`), but the tags
# do (`v16.0.3-0`). Stripping any `v` before prepending one keeps this correct
